...
contract SwapRouter is ISwapRouter {
    ...
    // zeroForOne true 有2种情况： token0 买入 token1  / token0 卖出换取 token1 
    // 大于 0 代表我们指定了要支付的 token0 的数量，amountSpecified 小于 0 则代表我们指定了要获取的 token1 的数量
    // sqrtPriceLimitX96 : 是交易的一个价格下限
    // data :  用户给合约转入代币的回调方法 的参数
    function swapInPool(
        IPool pool,
        address recipient,
        bool zeroForOne,
    ...
    // 买币
    // address tokenIn : 输入到 pool 的代币地址
    // address tokenOut :  从pool 拿走的代币地址
    // uint32[] indexPath :
    // address recipient : nft 收入地址
    // uint256 deadline :
    // uint256 amountIn :
    ...
    function exactOutput(
        ...
        for (uint256 i = 0; i < params.indexPath.length; i++) {
            ...
            (int256 amount0, int256 amount1) = this.swapInPool(
                ...
                params.recipient,
                zeroForOne,
                -int256(amountOut), // 只有这个地方和 exactInput 不同
                params.sqrtPriceLimitX96,
                data
            );
